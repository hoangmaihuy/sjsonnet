...
class PrettyYamlRenderer(out: Writer = new java.io.StringWriter(),
  ...
  val loadedFileContents: mutable.HashMap[Path,Array[Int]] = mutable.HashMap.empty[Path, Array[Int]]
  ...
  override def visitObject(length: Int, jsonKeys: Boolean, index: Int): ObjVisitor[Writer,Writer] = new ObjVisitor[Writer, Writer] {
    firstElementInArray = false
    var empty = true
    flushBuffer()
    if (!topLevel) depth += 1
...
object PrettyYamlRenderer{
  ...
  def stringNeedsToBeQuoted(str: String): Boolean = {
    ...
    def yamlNumber0[$: P] = P( ".inf" | yamlFloat | yamlOctalHex | CharIn("1-9") ~ digits.? | "0" )
